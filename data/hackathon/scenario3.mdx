...
Add a vLLM model server called `model-speedrun-[team name]` that uses a data connection to your cluster's object storage and the subfolder of its `models` bucket that contains your Granite model.
...
<Zoom>
|![model deploy](/hackathon/static/images/hackathon/model-deploy.png)           |
|:-----------------------------------------------------------------------------:|
| *Keep an eye on your model deployment progress*                               |
</Zoom>
...
To make the an inference call to your model use the `v1/chat/completions` Swagger `POST` interface, adapting the below payload as required:
...
```json
{
  ...
  "messages": [
    {
      "role": "system",
      "content": "You are Granite, a large language model trained on OpenShift AI."
    },
    {
      "role": "user",
      "content": "Tell me about the history of New York City."
    }
  ],
  "temperature": 0.7,
  "max_tokens": 150,
